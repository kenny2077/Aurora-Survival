...
TrailGuard keeps production and development trust separate:
...
xcodebuild -project TrailGuard.xcodeproj -scheme TrailGuard \
  -configuration Debug -sdk iphonesimulator \
  -destination 'generic/platform=iOS Simulator' \
  -derivedDataPath /tmp/TrailGuardDebugTrust \
  CODE_SIGNING_ALLOWED=NO build

xcodebuild -project TrailGuard.xcodeproj -scheme TrailGuard \
  -configuration Release -sdk iphonesimulator \
  -destination 'generic/platform=iOS Simulator' \
  -derivedDataPath /tmp/TrailGuardReleaseTrust \
  CODE_SIGNING_ALLOWED=NO build
```
