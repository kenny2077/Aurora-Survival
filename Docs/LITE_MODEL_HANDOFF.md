...
| Historical fuel-hazard control path | Superseded by Lite Chat architecture | `/tmp/TrailGuardGemmaSafetyBypassPhysical.xcresult` |
| Historical Apple Vision OCR path | Superseded; rerun with Lite Chat | `/tmp/TrailGuardOCRSafetyPhysical.xcresult` |
...
| Gate | Result | Evidence |
| --- | --- | --- |
| Exact grounded Manual navigation | Pass: “Where can I find water?” generated a grounded answer and opened “Locate Likely Water” | `/tmp/TrailGuardStatelessLitePhysical-exact-link.xcresult` |
| Stale-topic sequence | Pass: water answer followed by “How are you doing?” produced an ordinary greeting with no Manual link or water advice | `/tmp/TrailGuardStatelessLitePhysical-retry.xcresult` |
| Conditional repair failure | Pass: “How do I make water safer?” remained invalid after the single repair and showed the incomplete-answer terminal response with no link | `/tmp/TrailGuardStatelessLitePhysical-link-retry.xcresult` |
| Signed substantive package install | Pass: `model-lite-gemma3-1b-q4km-dev@1.1.0` installed and activated with the 160-token manifest | `/tmp/TrailGuardSubstantivePackageInstall.xcresult` |
| Generic car clarification | Safe failure: both generations were rejected and no procedure or Manual link appeared; the expected clarification copy was not produced | `/tmp/TrailGuardSubstantivePhysical/Logs/Test/Test-TrailGuard-2026.08.09_23-26-20-+0800.xcresult` |
...
| Simulator build and focused shell flows | Pass | `/tmp/TrailGuardDatabaseFirstBuild`, `/tmp/TrailGuardDatabaseFirstSimulator` |
| Native 16-case generation | Blocked: pinned b9637 `llama-completion` is absent | — |
| Physical five-grounded/five-fallback matrix | Blocked before inference: UI test runner killed during launch; retry hit `DebuggerVersionStore.StoreError` | `/tmp/TrailGuardDatabaseFirstPhysicalRetry.xcresult` |
