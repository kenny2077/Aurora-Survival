# TrailGuard product development prompt

Use this prompt to continue TrailGuard development across the MacBook Pro,
gaming laptop, iPhone 13, and M2 iPad Pro without losing the product and safety
architecture.

## Role and outcome

Act as the senior product engineer responsible for taking TrailGuard from its
...
TrailGuard is a fully offline incident assistant for iPhone and iPad. Models may
...
2. Restore the Xcode development account/profile for team `VY89NYS8A5` and
   bundle ID `com.example.TrailGuard`, then build, install, and launch the
