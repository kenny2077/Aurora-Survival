#!/usr/bin/env python3
"""Prepare locally hosted, signed TrailGuard product packages and catalog."""
    ...
    with tempfile.TemporaryDirectory(prefix="trailguard-product-knowledge-") as temp:
        ...
        for domain, display_name in names.items():
            ...
            source = {
                ...
                "expires_at": None,
                "license": {
                    "identifier": "LicenseRef-TrailGuard-Curated-Sources",
...
def prepare_map_package(
    ...
    if not source.is_file():
        raise FileNotFoundError(f"Map extraction is not ready: {source}")
    with tempfile.TemporaryDirectory(prefix=f"trailguard-map-{slug}-", dir=root) as temp:
        ...
        legal_dir.mkdir()
        pmtiles_path = map_dir / f"{slug}.pmtiles"
        try:
            os.link(source, pmtiles_path)
        except OSError:
            shutil.copy2(source, pmtiles_path)
        ...
        (style_dir / "style.json").write_bytes(canonical_json(style))
...
def main() -> None:
    ...
    twin = prepare_map_package(
        ...
        private_key,
        "twin-cities",
        "Twin Cities Offline Map",
        args.root / "maps-source" / "twin-cities.pmtiles",
        (-93.65, 44.70, -92.75, 45.30),
        "Twin Cities",
        "scout",
        args.map_font_assets,
        args.rebuild_maps,
    )
    minnesota = prepare_map_package(
        args.root,
        private_key,
        "minnesota",
        "Minnesota Offline Map",
        args.root / "maps-source" / "minnesota.pmtiles",
        (-97.50, 43.40, -89.40, 49.40),
        "Minnesota",
        "scout",
        args.map_font_assets,
        args.rebuild_maps,
    ...
    entries.extend(
        [
            catalog_entry(
                twin,
                "Compact street-level coverage for Minneapolis–Saint Paul and the metro.",
                {"region": "Twin Cities", "size_class": "small"},
            ),
            catalog_entry(
                minnesota,
                "Statewide roads, water, settlements, parks, and regional context.",
                {"region": "Minnesota", "size_class": "large"},
            ),
        ]
    )
